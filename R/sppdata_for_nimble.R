# sppdata_for_nimble <- function(sp.code,
#                                species.data,
#                                covar,
...
#   counter <- 1
#   # do PO ----
#   POind <- which(types.use == "PO")
#   if (length(POind) > 0) {
#
#     # __ Process iNat ----
#     inat.ind <- grep("iNaturalist", summary.sp$Name)
#
...
#       con.ext <- which(summary.sp$PO.extent[other.ind] == "CONUS")
...
#       sta.ext <- which(summary.sp$PO.extent[other.ind] != "CONUS")
...
#           dat$state <- summary.sp$PO.extent[sta.ind[o]]
#
#           sta.dat <- bind_rows(sta.dat, dat)
#         }
#         name <- paste0(summary.sp$Name[sta.ind], collapse = ", ")
#
#         # Filter dataset
...
#
#   # do DND ----
#   DNDind <- which(types.use == "DND")
#   if (length(DNDind) > 0) {
#     for (d in 1:length(DNDind)) {
#
#       # pull out covariates for this dataset
#       dnd.covs.mean <- unlist(strsplit(summary.sp$Covar.mean[DNDind[d]], split = ", "))
#       covs.mean <- dnd.covs.mean[which(is.na(dnd.covs.mean) == F)]
#
#       dnd.covs.sum <- unlist(strsplit(summary.sp$Covar.sum[DNDind[d]], split = ", "))
...
#
#   # do count ----
#   countind <- which(types.use == "Count")
