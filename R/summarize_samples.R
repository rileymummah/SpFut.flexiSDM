...
#' @param effort (logical) whether to summarize effort parameters (T) or not (F)
#' @param cores (numeric) designate the number of cores to parallelize the summarization. We suggest using parallel::detectCores() to determine an appropriate number of cores.
...
summarize_samples <- function(samples,
  ...
  cat(paste0("Summarizing chains using ",core," cores...\n"))

  start <- Sys.time()
  this_cluster <- makeCluster(core)
