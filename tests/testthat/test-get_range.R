...
test_that("get_range() makes list", {
  ...
  expect_true(file.exists('~/GitHub/species-futures/data/species/GPOR/GAP/'))

  expect_type(get_range(range.path = c(paste0("~/GitHub/species-futures/data/species/GPOR/GAP/"),
...
test_that("get_range() works with variable number of inputs", {
  tmp <- get_range(range.path = c(paste0("~/GitHub/species-futures/data/species/GPOR/GAP/"),
                                  paste0("~/GitHub/species-futures/data/species/GPOR/IUCN/")),
                   range.name = c("GAP", "IUCN"), crs = 4326)
