...
# 
# 
#   grid <- st_make_grid(boundary, cellsize = 100000) %>% st_as_sf() %>% mutate(conus.grid.id = 1:nrow(.))
# 
...
# 
# 
#   grid <- st_make_grid(st_transform(boundary, crs = 3857), cellsize = 100000, square = F) %>% st_as_sf() %>% mutate(conus.grid.id = 1:nrow(.))
...
# 
#   boundary <- rangelist[[1]]
#   grid <- st_make_grid(st_transform(boundary, crs = 3857), cellsize = 10000) %>% st_as_sf() %>% mutate(conus.grid.id = 1:nrow(.))
# 
#   region <- make_region(rangelist,
#                         buffer = 1,
#                         sub = F,
